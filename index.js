...
const find = (ident, name) => {
  return new Promise((yes, no) => {
    ...
    lister.on("data", function(data) {
      yes(!!data.Contents.find(i => i.Key == `recordings/${ident}/${name}`));
    });
  });
};
...
const checkDetails = res => async () => {
  let details = await fetch("https://freshair.org.uk/api/broadcast_info").then(
    r => r.json()
  );
...
const start = () => {
  ...
  https
    .get("https://radio.freshair.org.uk/radio", res => {
      ...
      int = setInterval(checkDetails(res), 3000);
    })
    .on("end", e => {
      console.log("end");
      clearInterval(int);
